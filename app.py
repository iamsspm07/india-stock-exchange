...
def forecast_stock_prices(df, days=7):
    ...
    future_days = []
    last_date = df.index[-1]
    while len(future_days) < days:
        last_date += pd.Timedelta(days=1)
        if last_date.weekday() < 5:  # Exclude weekends
            future_days.append(last_date)
    ...
    # Append forecasted data to DataFrame
    forecast_df = pd.DataFrame({'Date': future_days, 'Closing Price': forecast})

    # Plot combined data
    fig, ax = plt.subplots(figsize=(12, 8))
    ax.plot(df.index, df['Close'], label='Historical Prices', color='blue')
    ax.plot(forecast_df['Date'], forecast_df['Closing Price'], label='Forecasted Prices', color='red', linestyle='--', marker='o')
    ax.set_xlabel('Date')
    ax.set_ylabel('Closing Price')
    ax.set_title(f'Stock Prices with {days}-Day Forecast')
    ax.legend()
    st.pyplot(fig)
    ...
    if st.button('Get Data'):
        if symbol:
            ...
            if df is not None:
                ...
                # Show the forecast section
                st.write(f"### {forecast_days}-Day Stock Price Forecast")
